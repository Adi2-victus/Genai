...
// 📄 Read file
async function readFiles({ file_path }) {
  const content = fs.readFileSync(file_path, "utf-8");
  console.log(`📖 Reading: ${file_path}`);
  return { content };
}
...
const listFilesTool = {
  name: "list_files",
  description: "Get all HTML, CSS, JS, TS files in a directory",
  parameters: {
    type: Type.OBJECT,
    properties: {
      directory: {
...
const writeFileTool = {
  name: "write_file",
  description: "Write corrected content to file",
  parameters: {
    type: Type.OBJECT,
    properties: {
      file_path: {
  ...
  const History = [
    {
      ...
      parts: [
        {
          text: `Review and fix all code in directory: ${directoryPath}`,
        },
      ],
    },
  ];
  ...
  while (true) {
    ...
    if (result.functionCalls?.length > 0) {
      for (const functionCall of result.functionCalls) {
        ...
        const toolResponse = await toolsMap[name](args);
